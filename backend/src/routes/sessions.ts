...
router.get('/client/:clientId', async (req: AuthRequest, res) => {
  try {
    const sessions = await Session.find({ clientId: req.params.clientId, userId: req.user!.id });
    const sessionsWithId = sessions.map(session => ({ ...session.toObject(), id: session._id.toString() }));
    res.json(sessionsWithId);
  } catch (error) {
    res.status(500).json({ message: 'Server error' });
...
router.put('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    const session = await Session.findOneAndUpdate({ _id: req.params.id, userId: req.user!.id }, req.body, { new: true });
    if (!session) {
      return res.status(404).json({ message: 'Session not found' });
    }
